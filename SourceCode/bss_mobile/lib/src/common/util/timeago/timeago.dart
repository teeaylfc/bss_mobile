...
String format(DateTime date, String hhmm, String dateString, {String locale, DateTime clock, bool allowFromNow}) {
  ...
  String prefix, suffix, suffixAgoDay;
  ...
  if (seconds < 45)
    ...
    result = messages.days(days.round()) + ' ' + suffixAgoDay + ' ' + hhmm;
  else if (days < 30)
    result = messages.days(days.round()) + ' ' + suffixAgoDay + ' ' + dateString;
  else if (days < 60)
    result = messages.aboutAMonth(days.round()) + ' ' + suffixAgoDay + ' ' + dateString;
  else if (days < 365)
    result = messages.months(months.round()) + ' ' + suffixAgoDay + ' ' + dateString;
  else if (years < 2)
    result = messages.aboutAYear(months.round()) + ' ' + suffixAgoDay + ' ' + dateString;
  else
    result = messages.years(years.round());

  return [prefix, result].where((str) => str != null && str.isNotEmpty).join(messages.wordSeparator());
}
